...
app.get('/', async (c) => {
    return c.text('KCA Auth is Active. Dashboard is loading...', 200);
});
...
app.post('/license', async (c) => {
    try {
        ...
        const body = await c.req.json();
        let { key, license, secret, appName, appVersion, hwid } = body;
        const licenseKey = (key || license)?.trim();
        secret = secret?.trim(); appName = appName?.trim(); appVersion = appVersion?.trim(); hwid = hwid?.trim();

        if (!licenseKey || !secret || !appName || !appVersion) {
            return sdkJson(c, 'license', body, false, 'Missing required fields', {}, 400);
        }

        const appData = await getAppMetadata(c, secret, appName);
        if (!appData) return sdkJson(c, 'license', body, false, 'Application not found', {}, 404);
        if (appData.applicationPaused) return sdkJson(c, 'license', body, false, 'APPLICATION_PAUSED', {}, 403);
        if ((appData.version || 'UNKNOWN') !== appVersion) {
            return sdkJson(c, 'license', body, false, 'VERSION_MISMATCH', { serverVersion: appData.version || 'UNKNOWN' }, 409);
        }

        const userDataRaw = await dbRequest(c.env, `applications/${secret}/${appName}/users/${licenseKey}`);
        
        if (userDataRaw) {
            let userData = typeof userDataRaw === 'string' ? decrypt(userDataRaw, c.env.DB_SECRET) : userDataRaw;
            if (userData.isBanned) return sdkJson(c, 'license', body, false, 'USER_BANNED', {}, 403);

            if (userData.hwidLock) {
                if (!userData.sid) {
                    if (hwid) {
                        userData.sid = hwid;
                        await dbRequest(c.env, `applications/${secret}/${appName}/users/${licenseKey}`, 'PUT', encrypt(userData, c.env.DB_SECRET));
                    }
                } else if (userData.sid !== hwid) {
                    return sdkJson(c, 'license', body, false, 'HWID_MISMATCH', {}, 403);
                }
            }

            if (userData.expiry && userData.expiry !== 'lifetime') {
                if (new Date(userData.expiry) < new Date()) {
                    return sdkJson(c, 'license', body, false, 'LICENSE_EXPIRED', {}, 403);
                }
            }

            return sdkJson(c, 'license', body, true, 'LOGIN_SUCCESS', {
                username: licenseKey,
        ...
        const licenseDataRaw = await dbRequest(c.env, `applications/${secret}/${appName}/licenses/${licenseKey}`);
        if (!licenseDataRaw) return sdkJson(c, 'license', body, false, 'Invalid License Key', {}, 404);
        
        let licenseData = typeof licenseDataRaw === 'string' ? decrypt(licenseDataRaw, c.env.DB_SECRET) : licenseDataRaw;
        if (licenseData.used && licenseData.associatedUser !== licenseKey) {
            return sdkJson(c, 'license', body, false, 'License key already used', {}, 409);
        }
        ...
        await dbRequest(c.env, `applications/${secret}/${appName}/users/${licenseKey}`, 'PUT', encrypt(newUser, c.env.DB_SECRET));
        licenseData.used = true;
        ...
        return c.json(sendResponse(false, 'Internal Server Error'), 500);
    }
});
